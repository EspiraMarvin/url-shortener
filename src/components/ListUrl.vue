...
<!--        <q-list  class="full-width" style="max-width: 1200px"  square v-for="(n, index) in 6" :key="index">-->
        ...
        <q-list bordered separator class="full-width" style="max-width: 1200px" v-for="url in urlsList" :key="url._id">
          <q-item class="row">
            ...
            <q-item-section class="xs-hide sm-hide">
              <div style="width: 70%;  text-overflow: ellipsis;" class="col-lg-8 overflow-hidden ellipsis xs-hide sm-hide" :class="$q.screen.lt.md ? 'hidden' : ''">
                {{ url.longUrl }}
              </div>
              <q-tooltip>
                {{ url.date }}
            ...
            <q-space class="xs-hide sm-hide" />
            <q-item-section :side="$q.screen.gt.xs" class="flex items-start" ><a :href="url.shortUrl" class="text-blue"  target="_blank" style="text-decoration: none">{{ url.shortUrl }}</a></q-item-section>
